...
#define DEVICE_NAME                     "SmartFan"                              /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME               "ChaoticFocus"                          /**< Manufacturer. Will be passed to Device Information Service. */
...
{
    { FAN_SERVICE_UUID, BLE_UUID_TYPE_VENDOR_BEGIN }
};
 ...
 *
 */
static void on_fan_evt(ble_fan_t * p_fan_service, ble_fan_evt_t * p_evt) {
...
void services_init(void) {
    ...
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&fan_init.custom_value_char_attr_md.write_perm); 

    err_code = ble_fan_init(&m_fan, &fan_init); 
    APP_ERROR_CHECK(err_code);
}
